...
交互事件归一是 HarmonyOS 人机交互的核心机制，将不同输入设备（触屏手指、键盘+鼠标、键盘、触控板）的交互行为统一归一为标准事件，保证应用在不同输入方式下提供一致的交互体验。

覆盖的应用场景包括：悬浮、点击、双击、长按、上下文菜单、拖拽、滚动/平移、轻扫、缩放对象、旋转对象等。开发者无需为每种输入设备单独适配，系统自动将各类输入归一到对应的标准交互事件。

> 2026 年 2 月 5 日更新：新增触屏手指输入、键盘+鼠标输入、键盘输入、触控板输入动效示例及缩放图片；更新各应用场景输入设备/方式及交互行为参数。
...
| 多窗口交互 | 分屏、悬浮窗等窗口管理交互 |
| 服务卡片 | 桌面/负一屏的服务卡片展示与交互，包含待机屏保、锁屏卡片、互动卡片等 HarmonyOS 特征卡片 |
| 画中画 | 小窗播放视频的交互规范 |
| 深色模式 | 深色主题配色与适配规范 |
| 状态栏 | 顶部状态栏的适配与交互 |
| 播控中心 | 媒体播放控制中心交互 |
